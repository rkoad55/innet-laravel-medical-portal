...
<body class="hold-transition login-page">
  ...
  <div class="login-box">
    ...
    <img src="{{asset('images/logo.png')}}" alt="innet" title="innet" class="mainlogo" />
      <a href="" style="display:block"><b>Doctor </b>Registration</a>
    </div>
    <!-- /.login-logo -->
    <div class="card">
      ...
      @endif
        ...
        <form action="{{route('doctor.register.post')}}" method="post" role="form">
        @csrf
  ...
  <div class="input-group-append">
    <div class="input-group-text @error('email')text-danger @enderror">
      <span class="fas fa-envelope"></span>
          ...
          <!-- ------------------------------------------------------------------------------------------------------------ -->
          <?php 
          if(old('specilitization', null) != null){
            $specilitization =  old('specilitization');
          }
          $specilitization_return = array();
          if(is_array(old('specilitization')) && count(old('specilitization'))){
             $specilitization_return = old('specilitization');
           }
           ?>     
          <div class="form-group input-group mb-3 select2-danger">
                    <select class="select2" multiple="multiple" value="{{old('specilitization')}}" name="specilitization[]" data-placeholder="SELECT DEPARTMENT" data-dropdown-css-class="select2-danger" style="width: 100%;">
                    @foreach($specilitization as $key => $value)  
                    <option {{ (in_array($value['id'], $specilitization_return))?'selected=selected':'' }} value="{{$value['id']}}">{{$value['title']}}</option>
                    @endforeach  
                     
                    </select>
                    
                </div>
                @error('specilitization')
                  <p class="help-block text-danger">
                    <strong>{{$message}}</strong>
                </p>
            @enderror
