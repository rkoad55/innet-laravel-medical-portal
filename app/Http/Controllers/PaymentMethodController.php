...
class PaymentMethodController extends Controller
{
  ...
  public function _doPost($query) {
    ...
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, "https://secure.retrievermedgateway.com/api/transact.php");
  ...
  public function ChargePayment(Request $request) {
    ...
    // $this->setBilling("John","Smith","Acme, Inc.","123 Main St","Suite 200", "Beverly Hills",
    //         "CA","90210","US","555-555-5555","555-555-5556","support@example.com",
    //         "www.example.com");
    // $this->setShipping("Mary","Smith","na","124 Shipping Main St","Suite Ship", "Beverly Hills",
    //         "CA","90210","US","support@example.com");
    // $orderid,$orderdescription,$tax,$shipping,$ponumber,$ipaddress
    // $this->setOrder("1234","Appointment Book",1, 2, "PO1234","65.192.14.10");
  ...
  public function patientPayment($response, $cc_number, $cvv_number){
      if($response['id'] == 1 && $response['text'] == 'SUCCESS' && $response['code'] == 300){
        $payment = new Payment;
        $payment->patient_id = Auth::guard('patient')->user()->id;
        $payment->cc_number = $cc_number;
        $payment->cvv_number = $cvv_number;
        $payment->save();
      }
